...
const columns = [
    ...
    { field: "subject", headerName: "Name", minWidth: 150 },
...
const HireTutor = () => {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`${BASE_URL}/api/admin/hire-tutor-list-status/`)
            .then((res) => res.json())
            .then((data) => {
                const formattedData = data.map((item) => ({
                    ...item,
                    
                    created_at: moment(item?.created_at).format('YYYY-MM-DD'),  // Formatted Created Date
                    formattedJoinDate: moment(item?.created_at).format('DD/MM/YYYY hh:mm a') || '',  // Formatted Join Date
                    start_date: moment(item?.start_date).format('DD/MM/YYYY'),  // Formatted Start Date
                    subject: item?.subject?.join(", ") || "No Subjects",  // Display subjects as a comma-separated string
                    start_immediate: item?.start_immediate ? "Yes" : "No",  // Yes/No for start_immediate
                    is_approved: item?.is_approved ? "Approved" : "Not Approved",  // Approved/Not Approved
                    is_accepted: item?.is_accepted ? "Accepted" : "Not Accepted",  // Accepted/Not Accepted
                    is_paid: item?.is_paid ? "Paid" : "Not Paid",  // Paid/Not Paid
                ...
                >
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                            <Box>
                                <Typography variant="h6" sx={{ fontWeight: "bold" }}>
                                    {view?.job_title || "No Data"}
                                </Typography>
                                <Typography variant="body2" sx={{ color: "#555" }}>
                                    {view?.education_level || "No Data"} | {view?.curriculum || "No Data"}
                                </Typography>
                            </Box>
                        </Box>
                        <Box>
                            <Typography variant="body2" sx={{ textAlign: "right", color: "#777" }}>
                                <strong>Applied Tutor ID:</strong> {view?.applied_tutor_id || "No Data"}
                            </Typography>
                            <Typography variant="body2" className="mt-2">
                                <strong>Created At:</strong> {view?.created_at ? new Date(view.created_at).toLocaleString() : "No Data"}
                            </Typography>
                        </Box>
                    </Box>
                    ...
                    <Box sx={{ marginTop: 3 }}>
                        ...
                        >
                            ...
                            <Typography variant="body1">
                                <strong>Start Date:</strong> {view?.start_date || "No Data"}
                            </Typography>
                            <Typography variant="body1">
                                <strong>Days Per Week:</strong> {view?.days_per_week || "No Data"}
                            </Typography>
                            <Typography variant="body1">
                                <strong>Budget:</strong> {view?.budget || "No Data"}
                            </Typography>
                            <Typography variant="body1">
                                <strong>Lesson Type:</strong> {view?.lesson_type || "No Data"}
                            </Typography>
                            <Typography variant="body1">
                                <strong>Start Immediately:</strong> {view?.start_immediate ? "Yes" : "No"}
                            </Typography>
                            <Typography variant="body1">
                                <strong>Additional Comment:</strong> {view?.additional_comment || "No Data"}
                            </Typography>
                            <Typography variant="body1">
                                <strong>Subjects:</strong> {view?.subject || "No Data"}
                            </Typography>
                        </Box>
                    </Box>
